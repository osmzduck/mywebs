...
certificates.forEach(certificate => {
  ...
  certificate.addEventListener('mouseenter', () => {
    gsap.to(certificate, { duration: 0.3, scale: 1.05, rotationX: 10, rotationY: 10, ease: 'power1.inOut' });
  });

  certificate.addEventListener('mouseleave', () => {
    gsap.to(certificate, { duration: 0.3, scale: 1, rotationX: 0, rotationY: 0, ease: 'power1.inOut' });
  });
});
...
function animateTimelineBlock(entries) {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      entry.target.classList.add('active'); 
    }
